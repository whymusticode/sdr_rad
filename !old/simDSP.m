...
% reset(gpuDevice)
%% params
fsMain = 160e6; % sample rate
cpi = .5; % seconds integration
ds = 20; % downsample rate
bw = .25e6; % bandwidth
p.fc = 6e8; % center frequency
...
%% preliminary stuff
p.fs = fsMain/ds; % sample frequency
N = cpi*fsMain;
...
FILTER = fft(gS(fir1(1e4,bw/fsMain))',N);
...
data = fft(data);
dat = zeros(n0,2,nCh,'single','gpuArray');
for i = 1:nCh
  for j = 1:2
    if dConv(i)>0
      down = ifft([data(end-dConv(i)+1:end,j);data(1:end-dConv(i),j)].*FILTER);
    else
      down = ifft([data(-dConv(i)+1:end,j);data(1:-dConv(i),j)].*FILTER);
    end
    dat(:,j,i) = down(1:ds:end,:,:);
  end
end
