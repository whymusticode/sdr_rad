function pile = bistatDisambig(det,rangeErr,dopErr)
% load det

maxN = 2e6; % 4gb gpu ram
...
nReq = 4; % number of emitter detections required 
comboIdx = nchoosek(1:nE,nReq); % choose nReq emitters
nCombs = sum(prod(nPerEmit(comboIdx),2));
if nCombs > maxN
  disp(nCombs)
...
%% neck down
neckDown = .1; % use only the least error combinations
...
errS = sort(err2);
idx = err2 <= errS(round(length(errS)*neckDown));
...
pV = permute([XYZ,XYZvel],[2,1,3]);
y = [r/rangeErr;d/dopErr];
...
bigDetMat = [];
for i = 1:length(det)
  bigDetMat = [bigDetMat;det(i).rrd]; %#ok<AGROW>
end
Sall = zeros(nE,3);
...
%     tmp = alle(matchDetEm(i,:));
%     if length(tmp) > 1
%       potentAddOn = [potentAddOn;tmp']; %#ok<AGROW>
%     end
%   end
