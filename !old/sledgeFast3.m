function [ra,DOPS,rdi] = sledgeFast3(rx,tx,p)
% needs windowing 
...
DOPS = 0:c/cpi/p.fc:p.vMax; % gives me velocity
...
% number of batches can't be less than number of doppler bins
% optimally this is an integer number of doppler bins and power of 2


nB = 1000; % number of batches, do this intelligently
% more batches means we can make more velocity bins, fewer range bins
% more bins improves batch approximation
Nb = n/nB; % length of batches

sr = reshape(rx,[Nb,nB]);
sref = reshape(conj(tx),[Nb,nB]);
iter1 = ifft(fft(sr).*fft(flipud(sref)));
rdi = fft(iter1(1:nRan,:),[],2);
...
% insert = [ref(end);ref(1:n-1)];
% % insert = [n;(1:n-1)'];
% tmp = repmat(insert,[1,nRan+1]); 
% rows = n-1;
% cols = floor(numel(tmp)/rows);
% toep = reshape(tmp(1:rows*cols),[rows, cols]);
% toepe = [toep(:,1:nRan);flipud(insert(end-nRan+1:end))'];
% sref = reshape(toepe,[Nb,nB,nRan]);
% 
% ccfs = sum(sref.*sr,1);
% rdi = permute(fft(ccfs,[],2),[2,3,1]);
% rdi = [rdi(end-floor(nDop/2)+1:end,:);rdi(1:round(nDop/2),:)];
...
%% slow way of indexing
% P = (1:Nb)';
% r = permute(0:nB-1,[1,3,2]);
% l = 1:nRan;
% rnb = r*Nb;
% idx1 = rnb+P;
% idx2 = idx1 - l;
...
% tmp = repmat(ref,[1,nRan+1]); % this has all the values we need 
% rows = n+1;
% cols = floor(numel(tmp)/rows);
% toep = reshape(tmp(1:rows*cols),[rows, cols]);
% sref = reshape(toep(1:end-1,:),[Nb,nB,nRan]);


%% proof of reshaping concept
% reshape([(1:6)',(7:12)'],[3,2,2])
% idx1 = reshape(1:n,[Nb,nB]);
% tmp = repmat((1:4)',[1,5]);
% cols = floor(numel(tmp)/3);
% reshape(tmp(1:cols*3),[3,cols]);
