...
pV = permute(XYZ,[2,1,3]); % positionVelocity initialization

% yData = [r/rangeErr;d/dopErr];
yData = r/rangeErr;
totErr = zeros(n,2,'single','gpuArray');
for i = 1:4
  [yEst,jac] = HjacPos(pV,S,rangeErr,dopErr);
  res = yData-yEst;
...
hold on
tmp = permute(est,[3,1,2]);
plot(tmp(:,1)/1e3,tmp(:,2)/1e3,'go');
