...
addpath(genpath('C:\Users\MikeB\Desktop\CODE\SUBS'))
reset(gpuDevice)
clear
% addpath('data')
...
fc = 89.7e6; % Center frequency (Hz) [88.5]
fs  = 300e3; %(hz) 225e3 < fs <= 300e3 or 900e3 < fs <= 3200e3
...
while 1
  ...
  try % try to load the data
    ...
    if channelSwap; rx = [rx;DATA];else;tx = [tx;DATA];end
    load(['data\rx',num2str(i)])
    if channelSwap; tx = [tx;DATA];else;rx = [rx;DATA];end
 ...
 RXclean = removeDirectVect(winLen,15,TX,RX);
%  RXclean = ifft(removeDirectVect(winLen,5,fft(TX),fft(RXclean)));
 
  
%   RXclean = ifft(removeDirectVect(winLen,10,fft(TX),fft(RX)));
%   RXclean = removeDirectVect(winLen,10,TX,RXclean);
